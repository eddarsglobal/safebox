...
echo "==> Backup config"
cp src-tauri/tauri.conf.json "src-tauri/tauri.conf.json.backup-official-icon-fix.$(date +%Y%m%d%H%M%S)"

echo "==> Install official SBX icon source"
mkdir -p src-tauri/icons
cp "$PATCH_DIR/safebox-file-source.png" "src-tauri/icons/safebox-file-source.png"

echo "==> Force .sbx association"
python3 - <<'PY'
import json
...
mkdir -p scripts
cat > scripts/install_official_sbx_icon.sh <<'SH'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x scripts/install_official_sbx_icon.sh

echo "Done. Next:"
echo "cd \"$DESKTOP\""
echo "npm run build && npm run tauri build"
echo "bash scripts/install_official_sbx_icon.sh"
