...
<section class="ad-shell" id="adShell" aria-label="Advertisement" hidden><span data-i="ad"></span><div class="ad-slot" id="landing-ad" data-ad-state="disabled"></div></section>
...
// AdSense runs only on the public Web landing page, never in the crypto app or native builds.
if (import.meta.env.MODE === "web") {
  const shell = root.querySelector<HTMLElement>("#adShell")!;
  const slot = root.querySelector<HTMLElement>("#landing-ad")!;
  const ad = document.createElement("ins");
  ad.className = "adsbygoogle";
  ad.style.display = "block";
  ad.dataset.adClient = "ca-pub-3925930420157238";
  ad.dataset.adSlot = "2746924080";
  ad.dataset.adFormat = "auto";
  ad.dataset.fullWidthResponsive = "true";
  slot.replaceChildren(ad);
  slot.dataset.adState = "enabled";
  shell.hidden = false;
  const adsWindow = window as Window & { adsbygoogle?: { push(value: Record<string, never>): unknown } };
  adsWindow.adsbygoogle = adsWindow.adsbygoogle || [];
  try { adsWindow.adsbygoogle.push({}); }
  catch (error) { console.warn("AdSense did not initialise on the landing page", error); }
}
