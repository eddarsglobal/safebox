...
cd "$ROOT"
[[ "$(cat SAFEBOX_V028_PACKAGE_ID.txt)" == "v0.2.8-web-wasm-full-lock-resolution-r72-20260830A" ]] || { echo "SAFEBOX_WEB_R72_PACKAGE_ID_FAIL"; exit 1; }
echo "SAFEBOX_WEB_R72_PACKAGE_ID_PASS"
...
assert 'CARGO_NET_OFFLINE=true cargo metadata --format-version 1' in locksync
assert 'CARGO_NET_OFFLINE=true cargo metadata --locked --format-version 1' in locksync
assert 'cargo update -p argon2' not in locksync
assert 'SAFEBOX_WEB_CARGO_LOCK_EXPECTED_FEATURE_DELTA_PASS' in locksync
assert "('password-hash','0.5.0')" in locksync and "('base64ct','1.8.3')" in locksync
assert 'cargo tree --locked -p safebox-web-wasm --target "$TARGET"' in locksync
assert 'forbidden RNG dependency in wasm graph' in locksync
assert 'getrandom|rand_core' in locksync
assert 'web:lock-check' in pkg
print('SAFEBOX_WEB_R72_FULL_OFFLINE_RESOLUTION_POLICY_PASS')
print('SAFEBOX_WEB_R72_EXPECTED_FEATURE_DELTA_GUARD_PASS')
print('SAFEBOX_WEB_R72_NO_VERSION_DRIFT_GUARD_PASS')
print('SAFEBOX_WEB_R72_WASM_NO_RNG_GRAPH_GUARD_PASS')
PY
...
print('SAFEBOX_WEB_R72_SECURITY_IMMUTABLE_HASHES_PASS')
PY

if find . -type d \( -name node_modules -o -name target -o -name dist -o -name __pycache__ \) -print -quit | grep -q .; then
  echo "SAFEBOX_R72_HYGIENE_FAIL"; exit 1
fi
if find . -type f -name 'safebox_core.wasm' -print -quit | grep -q .; then
  echo "SAFEBOX_R72_HYGIENE_FAIL: prebuilt WASM present"; exit 1
fi

echo "SAFEBOX_R72_HYGIENE_PASS"
echo "SAFEBOX_V028_WEB_WASM_FULL_LOCK_RESOLUTION_R72_VERIFY_PASS"
