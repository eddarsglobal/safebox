...
fail(){ echo "SAFEBOX_V028_IOS_SHARE_R47_FAIL: $*" >&2; exit 1; }

printf '%s\n' '[1/8] package + baseline retained'
grep -Fq 'SAFEBOX_V028_PACKAGE_ID=v0.2.8-ios-native-share-extension-r47-20260829E' "$ROOT/SAFEBOX_V028_PACKAGE_ID.txt" || fail 'package id mismatch'
...
printf '%s\n' '[3/8] canonical Share plist contract'
python3 - "$INFO" <<'PY'
import plistlib,sys
with open(sys.argv[1],'rb') as f: p=plistlib.load(f)
assert p['NSExtension']['NSExtensionPointIdentifier']=='com.apple.share-services'
PY
printf '%s\n' 'SAFEBOX_IOS_SHARE_CANONICAL_POINT_R47_PASS'

printf '%s\n' '[4/8] XcodeGen must not generate/mutate Info.plist'
bash -n "$INSTALL"
! grep -Eq '^[[:space:]]+info:[[:space:]]*$' "$INSTALL" || fail 'xcodegen info generation block survived'
for token in 'INFOPLIST_FILE: Info.plist' 'GENERATE_INFOPLIST_FILE: NO' 'PLIST_SHA_BEFORE=' 'PLIST_SHA_AFTER=' 'xcodegen mutated static Share Extension Info.plist' 'SAFEBOX_IOS_SHARE_INFO_PLIST_PRESERVED_PASS'; do
  grep -Fq "$token" "$INSTALL" || fail "immutable plist token missing: $token"
done
printf '%s\n' 'SAFEBOX_IOS_SHARE_STATIC_PLIST_XCODEGEN_R47_PASS'

printf '%s\n' '[5/8] compiled appex extension-point gate'
bash -n "$SIM"
for token in 'ACTUAL_SHARE_POINT=' 'SAFEBOX_IOS_SHARE_EXTENSION_POINT:' 'com.apple.share-services' 'SAFEBOX_IOS_SHARE_EXTENSION_POINT_PASS'; do
  grep -Fq "$token" "$SIM" || fail "compiled plist gate token missing: $token"
done
printf '%s\n' 'SAFEBOX_IOS_SHARE_COMPILED_POINT_GATE_R47_PASS'

printf '%s\n' '[6/8] R46 standalone/App Group architecture retained'
for token in 'SAFEBOX_IOS_SHARE_NO_RUBY_PROJECT_MUTATION_PASS' 'SAFEBOX_IOS_SHARE_STANDALONE_XCODEGEN_PROJECT_PASS' 'SAFEBOX_IOS_SHARE_APP_GROUP_ENTITLEMENTS_PASS' 'SAFEBOX_IOS_SHARE_INBOX_BRIDGE_PASS'; do
  grep -Fq "$token" "$INSTALL" || fail "R46 architecture token missing: $token"
done
for token in 'SAFEBOX_IOS_SHARE_EXTENSION_BUILD_BEGIN' 'SAFEBOX_IOS_SHARE_EXTENSION_TARGET_PASS' 'merge_app_group_and_resign' 'SAFEBOX_IOS_SHARE_APP_GROUP_RUNTIME_ENTITLEMENTS_PASS' 'SAFEBOX_IOS_SHARE_EXTENSION_EMBED_PASS'; do
  grep -Fq "$token" "$SIM" || fail "R46 runtime token missing: $token"
done
printf '%s\n' 'SAFEBOX_IOS_SHARE_R46_ARCHITECTURE_RETAINED_PASS'
...
printf '%s\n' 'SAFEBOX_IOS_SHARE_RUNTIME_CHECKER_R47_PASS'

printf '%s\n' '[8/8] hygiene + report'
[[ -s "$ROOT/checkpoints/SAFEBOX_V028_IOS_NATIVE_SHARE_EXTENSION_R47_REPORT.md" ]] || fail 'R47 report missing'
...
printf '%s\n' 'SAFEBOX_IOS_SHARE_HYGIENE_R47_PASS'
printf '%s\n' 'IOS_V028_STATUS: NATIVE_SHARE_EXTENSION_R47_READY_FOR_RUNTIME_TEST'
printf '%s\n' 'SAFEBOX_V028_IOS_NATIVE_SHARE_EXTENSION_R47_VERIFY_PASS'
