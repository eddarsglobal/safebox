...
cd "$ROOT"
[[ "$(cat SAFEBOX_V028_PACKAGE_ID.txt)" == "v0.2.8-web-wasm-argon2-explicit-memory-r76-20260830A" ]] || { echo "SAFEBOX_WEB_R76_PACKAGE_ID_FAIL"; exit 1; }
echo "SAFEBOX_WEB_R76_PACKAGE_ID_PASS"
...
assert 'use argon2::{Algorithm, Argon2, Block, Params, Version};' in crypto
assert 'argon2.params().block_count()' in crypto
assert 'hash_password_into_with_memory' in crypto
assert 'hash_password_into(code.as_bytes()' not in crypto
assert 'memory_blocks.zeroize()' in crypto
assert 'key.zeroize()' in crypto
...
assert not re.search(r'(?m)^name = "password-hash"$', lock)
print('SAFEBOX_WEB_R76_ARGON2_EXPLICIT_MEMORY_PASS')
print('SAFEBOX_WEB_R76_ARGON2_NO_ALLOC_RNG_PATH_PASS')
print('SAFEBOX_WEB_R76_ARGON2_MEMORY_ZEROIZE_PASS')
print('SAFEBOX_WEB_R76_CRYPTO_PARAMETERS_RETAINED_PASS')
PY

bash -n safebox-desktop/scripts/web_cargo_lock_sync.sh
bash -n safebox-desktop/scripts/web_wasm_build.sh
python3 -m py_compile safebox-desktop/scripts/web_wasm_source_check.py
...
print('SAFEBOX_WEB_R76_SECURITY_IMMUTABLE_HASHES_PASS')
PY

if find . -type d \( -name node_modules -o -name target -o -name dist -o -name __pycache__ \) -print -quit | grep -q .; then
  echo "SAFEBOX_R76_HYGIENE_FAIL"; exit 1
fi
if find . -type f -name 'safebox_core.wasm' -print -quit | grep -q .; then
  echo "SAFEBOX_R76_HYGIENE_FAIL: prebuilt WASM present"; exit 1
fi

echo "SAFEBOX_R76_HYGIENE_PASS"
echo "SAFEBOX_V028_WEB_WASM_ARGON2_EXPLICIT_MEMORY_R76_VERIFY_PASS"
