...
cd "$ROOT"
[[ "$(cat SAFEBOX_V028_PACKAGE_ID.txt)" == "v0.2.8-web-wasm-lock-selfheal-r71-20260830A" ]] || { echo "SAFEBOX_WEB_R71_PACKAGE_ID_FAIL"; exit 1; }
echo "SAFEBOX_WEB_R71_PACKAGE_ID_PASS"
...
assert 'CARGO_NET_OFFLINE=true cargo metadata --locked' in locksync
assert 'cargo update -p argon2 --precise 0.5.3' in locksync
assert 'SAFEBOX_WEB_CARGO_LOCK_NO_VERSION_DRIFT_PASS' in locksync
assert 'allowed_removed' in locksync and 'password-hash' in locksync and 'base64ct' in locksync
...
print('SAFEBOX_WEB_R71_LOCK_SELFHEAL_POLICY_PASS')
print('SAFEBOX_WEB_R71_OFFLINE_REPAIR_PASS')
print('SAFEBOX_WEB_R71_NO_VERSION_DRIFT_GUARD_PASS')
print('SAFEBOX_WEB_R71_WASM_GRAPH_RUNTIME_GUARD_PASS')
PY
...
print('SAFEBOX_WEB_R71_SECURITY_IMMUTABLE_HASHES_PASS')
PY

if find . -type d \( -name node_modules -o -name target -o -name dist -o -name __pycache__ \) -print -quit | grep -q .; then
  echo "SAFEBOX_R71_HYGIENE_FAIL"; exit 1
fi
if find . -type f -name 'safebox_core.wasm' -print -quit | grep -q .; then
  echo "SAFEBOX_R71_HYGIENE_FAIL: prebuilt WASM present"; exit 1
fi

echo "SAFEBOX_R71_HYGIENE_PASS"
echo "SAFEBOX_V028_WEB_WASM_LOCK_SELFHEAL_R71_VERIFY_PASS"
