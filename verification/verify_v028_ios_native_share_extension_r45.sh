...
fail(){ echo "SAFEBOX_V028_IOS_SHARE_R45_FAIL: $*" >&2; exit 1; }

printf '%s\n' '[1/10] package + R42 baseline retained'
grep -Fq 'SAFEBOX_V028_PACKAGE_ID=v0.2.8-ios-native-share-extension-r45-20260829C' "$ROOT/SAFEBOX_V028_PACKAGE_ID.txt" || fail 'package id mismatch'
...
printf '%s\n' '[8/11] generated Xcode target installer'
bash -n "$INSTALL"
...
printf '%s\n' '[9/11] CocoaPods Ruby/xcodeproj resolver'
for token in 'POD_REAL=' 'RUBY_CANDIDATES=()' 'GEM_ROOT_CANDIDATES=()' 'GEM_HOME="$gem_root"' 'GEM_PATH="$merged"' 'run_xcodeproj_ruby()' 'SAFEBOX_IOS_SHARE_XCODEPROJ_RUNTIME_PASS: mode=cocoapods-bundled' 'run_xcodeproj_ruby - "$PROJECT"'; do
  grep -Fq "$token" "$INSTALL" || fail "xcodeproj resolver token missing: $token"
done
! grep -Fq "ruby -e 'require \"xcodeproj\"'" "$INSTALL" || fail 'legacy bare ruby guard survived'
printf '%s\n' 'SAFEBOX_IOS_SHARE_XCODEPROJ_RESOLVER_R45_PASS'

printf '%s\n' '[10/11] simulator/runtime gate'
...
printf '%s\n' '[11/11] hygiene + report'
[[ -s "$ROOT/checkpoints/SAFEBOX_V028_IOS_NATIVE_SHARE_EXTENSION_R45_REPORT.md" ]] || fail 'R44 report missing'
...
printf '%s\n' 'SAFEBOX_IOS_SHARE_HYGIENE_R45_PASS'
printf '%s\n' 'IOS_V028_STATUS: NATIVE_SHARE_EXTENSION_READY_FOR_RUNTIME_TEST'
printf '%s\n' 'SAFEBOX_V028_IOS_NATIVE_SHARE_EXTENSION_R45_VERIFY_PASS'
